    ...
    try:
        ...
        with open(raw_file, encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                bracket = (
                    row.get("faixa") or row.get("Faixa de Renda") or row.get("bracket")
                )
                declarations = (
                    row.get("quantidade")
                    or row.get("Quantidade de Declarações")
                    or row.get("count")
                )
                taxable_income = (
                    row.get("rendimento_tributavel")
                    or row.get("Rendimento Tributável")
                    or "0"
                )
...
def _write_fallback(output_file: str) -> str:
    ...
    for year, total_decl_m, total_tax_b in yearly:
        for i, (bracket_name, eff_rate) in enumerate(brackets):
            ...
            tax_share_normalized = tax_share / sum(
                d * (r / sum(rr for _, rr in brackets))
                for d, (_, r) in zip(bracket_dist, brackets)
            )
